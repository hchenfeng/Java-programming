...
public class Day {
    ...
    public Day(int aYear, int aMonth, int aDate) {
        if (aYear == 0) {
            throw new IllegalArgumentException("Year must not be 0.");
        } else if (aMonth < 1 || aMonth > 12) {
            throw new IllegalArgumentException("Month must be an int between 1 and 12.");
        } else if (aDate < 1 || aDate > 31) {
            throw new IllegalArgumentException("Date must be an int between 1 and 31.");
        } else {
            this.year = aYear;
            this.month = aMonth;
            this.date = aDate;
    ...
    public int daysFrom(Day other) {
        ...
        // set d to its previous day as long as d is after other
        // also count the number of times it is done
        ...
        // set d to its next day as long as d is before other
        // also count the number of times it is done
        else if (d.compareTo(other) < 0) {
            while (d.compareTo(other) != 0) {
                d = d.nextDay();
                n += 1;
            }
